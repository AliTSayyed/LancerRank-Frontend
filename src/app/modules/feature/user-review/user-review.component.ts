...
export class UserReviewComponent {
  ...
  fetchUser(id: string) {
    this.userService
      .getOneUser(`https://lancerrank-backend.onrender.com/api/users/${id}`)
      .subscribe((user: User) => {
        this.user = user;
      });
  }
  ...
  showAlert() {
    alert(
      'User and freelancer names may not show on review boxes since the site is hosted with Git-Hub-Pages.\nRefresh the page if you have this problem.'
    );
  }
